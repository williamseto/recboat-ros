...
void planxythetalat(char* envCfgFilename, char* motPrimFilename){
    ...
    env.GetEnvParms(&size_x, &size_y, &num_thetas, &startx, &starty, &starttheta, &goalx,
                                              &goaly, &goaltheta, &cellsize_m, &nominalvel_mpersecs,
                                              &timetoturn45degsinplace_secs, &obsthresh, &motionprimitiveV);


    setEnvStartGoal(env, startx, starty, starttheta, goalx, goaly, goaltheta, start_id, goal_id);
